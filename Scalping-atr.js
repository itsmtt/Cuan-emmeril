    ...
    while (true) {
      try {
        ...
        if (shortEMA > longEMA && rsi < 70) {
          ...
          const takeProfit = lastPrice + 1 * atr; // Take profit 1x ATR
          const stopLoss = lastPrice - 0.5 * atr; // Stop loss 0.5x ATR
  ...
  while (true) {
    try {
      ...
      const takeProfit = side === 'BUY'
        ? activeOrder.price + 1 * atr
        : activeOrder.price - 1 * atr;

      const stopLoss = side === 'BUY'
        ? activeOrder.price - 0.5 * atr
        : activeOrder.price + 0.5 * atr;

      console.log(`ATR dinamis: ${atr.toFixed(4)}, Take Profit: ${takeProfit.toFixed(4)}, Stop Loss: ${stopLoss.toFixed(4)}`);
